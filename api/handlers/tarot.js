...
const {
  startGame, placerPari, jouerCarte, resolveTrick, endRound, getPublicState,
  areBidsComplete, areTricksComplete
...
async function bid(room, body, res, games) {
  ...
  res.status(200).json({ success: true, phase: room.phase, gameState: getPublicState(room) });
}
...
async function nextTrick(room, body, res, games) {
  ...
  if (areTricksComplete(room)) {
    ...
    res.status(200).json({ success: true, pli: r, phase: room.phase, gameState: getPublicState(room) });
    return;
  }
  res.status(200).json({ success: true, pliResolu: r, phase: room.phase, gameState: getPublicState(room) });
}
